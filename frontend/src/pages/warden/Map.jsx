...
export default function Map() {
  ...
  const fetchActiveStudents = async () => {
    try {
      ...
      // Set center to the first student's location or default
  ...
  const connectWebSocket = () => {
    ...
    try {
      ...
      wsRef.current.onopen = () => {
        ...
        toastService.success('🔗 Connected to live tracking');
        ...
        try {
          ...
          if (message.type === 'location_update') {
            ...
            if (message.data.length > 0) {
              setCenter([
                message.data[0].latitude,
                message.data[0].longitude,
              ]);
            }
            toastService.info(`📍 Found ${message.data.length} active students`);
            ...
            if (status === 'closed' || status === 'rejected' || status === 'expired') {
              ...
              toastService.info(`🚫 Student request ${status}`);
            }
          }
        } catch (err) {
      ...
      wsRef.current.onerror = (error) => {
        ...
        setConnectionStatus('disconnected');
        toastService.error('⚠️ Connection lost to live tracking');
      };
      ...
      wsRef.current.onclose = () => {
        ...
        if (reconnectCountRef.current < MAX_RECONNECT_ATTEMPTS) {
          ...
          setConnectionStatus('disconnected');
          toastService.error(`🔌 Disconnected (${reconnectCountRef.current} reconnection attempts failed)`);
        }
      };
    } catch (err) {
