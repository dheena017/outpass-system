    ...
    <>
      ...
      <nav className={`hidden md:flex flex-col fixed top-0 left-0 h-screen z-40 overflow-y-auto bg-slate-900 border-r border-white/5 text-white w-72 p-6 transition-all duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="mb-10 p-5 rounded-2xl bg-gradient-to-br from-white/5 to-transparent border border-white/10 shadow-xl overflow-hidden relative group">
          <div className="absolute top-0 right-0 w-20 h-20 bg-blue-500/10 rounded-full blur-2xl -mr-10 -mt-10 transition-all group-hover:bg-blue-500/20"></div>
          <div className="flex items-center gap-4 relative z-10">
            <div className="w-12 h-12 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-lg shadow-lg ring-2 ring-white/10">
              {initials || 'U'}
            </div>
            <div className="min-w-0">
              <p className="text-[10px] text-blue-400 font-bold uppercase tracking-widest mb-1">Student Portal</p>
        ...
        <div className="space-y-4 mt-10 pt-8 border-t border-white/5">
          ...
          >
            ...
            <div className={`w-10 h-5 rounded-full relative transition-colors duration-300 ${dark ? 'bg-blue-600' : 'bg-gray-700'}`}>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 glass bg-slate-900/80 backdrop-blur-xl border-b border-white/5 h-16 flex items-center justify-between px-6 px-safe pt-safe">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-600 flex items-center justify-center text-white font-bold text-sm shadow-lg ring-2 ring-white/10">
            {initials || 'U'}
          </div>
        </div>
      </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 glass bg-white/70 dark:bg-slate-900/80 backdrop-blur-2xl border-t border-gray-200/50 dark:border-white/5 flex shadow-[0_-10px_30px_-10px_rgba(0,0,0,0.1)] pt-2 pb-safe px-4 px-safe">
          ...
          return (
            ...
            >
              <div className={`p-3 rounded-2xl transition-all duration-300 ${active
                ...
                <link.icon size={22} className={`transition-transform duration-300 ${active ? 'scale-110' : 'group-hover:scale-110'}`} />
              </div>
              <span className={`text-[10px] mt-2 font-bold tracking-widest uppercase transition-colors ${active ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400'}`}>
                {link.label.split(' ')[0]}
              </span>
              {active && (
                <span className="absolute -top-2 w-10 h-1 bg-blue-600 rounded-b-full shadow-[0_2px_10px_rgba(37,99,235,0.5)]"></span>
              )}
            </Link>
          );
        })}
        <button
          onClick={logout}
          className="flex-1 flex flex-col items-center justify-center py-2 text-gray-500 dark:text-gray-400 group"
        >
